        ...
        -------------------------->
     	...
     	<div class="col-md-12">
     		<div class="box box-primary"><!-- form에서 데이터를 전송할 때 enctype 기본값이지만 파일첨부가 포함되어 있으면 "multipart/form-data" 로 설정해야 한다 -->
     			<form id="productForm" action="/admin/product/modifyProd" method="post" enctype="multipart/form-data">
     			...
     			<div class="box-body">	
					  <div class="form-group row">
					    ...
					    <div class="col-sm-3">
					    <!-- 수정 버튼 클릭 후 이전의 검색기록과 페이지 정보의 list페이지로 이동하기 위해서 @ModelAttribute("cri") Criteria cri 를 이용하여 히든 폼 처리  -->
                ...
                <input type="hidden" class="form-control" id="prod_no" name="prod_no" value="${product.prod_no}">
					      <input type="text" class="form-control" id="prod_name" name="prod_name" value="${product.prod_name}">
					    </div>
					    <label for="prod_price" class="col-sm-2 col-form-label">상품가격</label>
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="prod_price" name="prod_price" value="${product.prod_price}">
					    </div>
					  </div>
					  <div class="form-group row">
					    <label for="prod_discount" class="col-sm-2 col-form-label">할인율</label>
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="prod_discount" name="prod_discount" value="${product.prod_discount}">
					    </div>
					    <label for="prod_company" class="col-sm-2 col-form-label">제조사</label>
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="prod_company" name="prod_company" value="${product.prod_company}">
					    </div>
					  </div>
					  <div class="form-group row">
					    <label for="pdt_img" class="col-sm-2 col-form-label">상품이미지</label>
					    <div class="col-sm-2"> <!-- name="pdt_img" 존재안함.  스프링에서는 Null 이 됨.-->
					    <!-- 사용자가 새로운 이미지로 변경하지 않을 경우 기존의 이미지 정보를 그대로 업데이트 -->
                <input type="hidden" name="prod_upload_folder" value="${product.prod_upload_folder}"><!-- 상품 등록할 때 사용한 이미지 저장 폴더 경로 -->
                <input type="hidden" name="prod_img" value="${product.prod_img}"><!-- 상품 등록할 때 사용한 이미지 파일명 -->
					      <input type="file" class="form-control" id="uploadFile" name="uploadFile">
					    </div>
              <label for="change_img" class="col-sm-2 col-form-label">현재 / 변경 이미지</label><!-- 업로드하기 위해 선택한 파일 중 첫번째 파일이 보여진다 -->
					    <div class="col-sm-3">
              ...
              <div class="col-sm-3">
					  ...
					  <div class="form-group row">
					    ...
					    <div class="col-sm-10">
					      <textarea class="form-control" id="prod_detail" name="prod_detail" rows="3">${product.prod_detail}</textarea>
					    </div>
					  </div>
					  <div class="form-group row">
					    <label for="prod_stock" class="col-sm-2 col-form-label">수량</label>
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="prod_stock" name="prod_stock" value="${product.prod_stock}">
					    </div>
