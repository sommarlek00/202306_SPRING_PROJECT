        ...
        -------------------------->
     	...
     	<div class="col-md-12">
     		<div class="box box-primary"><!-- form에서 데이터를 전송할 때 enctype 기본값이지만 파일첨부가 포함되어 있으면 "multipart/form-data" 로 설정해야 한다 -->
     			<form id="addUserForm" action="/admin/member/modifyUser" method="post">
                ...
                <div class="col-sm-4">
                  <button type="button" data-toggle="modal" data-target="#pwChangeModal" class="btn btn-outline-secondary" >재설정</button>
                </div>
					    </div>
					  ...
					  <div class="form-group row">
					    ...
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="user_email" name="user_email" value="${member.user_email}">
					    </div>
					  </div>
					  ...
					  <div class="form-group row">
					    ...
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="user_nickname" name="user_nickname" value="${member.user_nickname}">
					    </div>
			          ...
			          <div class="col-sm-4">
					    ...
					    <div class="col-sm-4">
					      <input type="text" class="form-control" id="user_phone" name="user_phone" value="${member.user_phone}" placeholder="예시) 01012345678">
					    </div>
					  </div>
					...
					<label for="sample2_postcode" class="col-sm-2 col-form-label">우편번호</label>
			          <div class="col-sm-4">
			            <input type="text" class="form-control" name="user_zipcode" id="sample2_postcode" value="${member.user_zipcode}"><!-- 다음 api에서 가져온 id -->
			          </div>
					...
					<label for="sample2_address" class="col-sm-2 col-form-label">주소</label>
			          <div class="col-sm-4">
			           <input type="text" class="form-control" name="user_addr" id="sample2_address" value="${member.user_addr}">
			          </div>
					<label for="sample2_detailAddress" class="col-sm-2 col-form-label">상세주소</label>
			          <div class="col-sm-4">
			            <input type="text" class="form-control" name="user_addr_detail" id="sample2_detailAddress" value="${member.user_addr_detail}" placeholder="상세주소를 입력해주세요.">
     			...
     			<div class="box-footer">
     				...
     				<div class="form-group row">
						  <div class="col-sm-12 text-center">
						  	<button type="submit" id="btn_modifyUser" class="btn btn-dark" id="btnProduct">회원정보 수정</button>
						  </div>			
					</div>
     			</div>
     			</form>
...
$(document).ready(function() {
  ...
  $("#btn_join").click(function(){
    ...
    // 유효성 검사 코드
      ...
      let user_id = $("#pwChange_user_id").data("user_id");
  		
  		$.ajax({
  			url: '/admin/member/changeUserPW',
  			type: 'post',
  			data: {new_user_password: $("#new_user_password").val(), user_id : user_id},
  			dataType: 'text',
  			success: function(result){
  				if(result == "success"){
      ...
      <div class="modal-body">
         <div class="row">
          <div class="col-md-6 mb-3"> <!-- 한 행에 여러 필드를 넣고 싶을 때 row, col-md 설정 -->
            ...
            <input type="hidden" id="pwChange_user_id" data-user_id="${member.user_id}">
          </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">취소</button>
        <button type="button" class="btn btn-primary" id="btn_pwChange">확인</button>
      </div>
    </div>
  </div>
</div>
